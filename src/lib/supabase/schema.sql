...
-- 3. Create a 'bids' table for real-time auctions
CREATE TABLE IF NOT EXISTS bids (
    ...
    status TEXT DEFAULT 'placed' CHECK (status IN ('placed', 'accepted', 'rejected', 'outbid'))
);
...
-- 8. Create Function to Validate Bids (Must be higher than current price)
CREATE OR REPLACE FUNCTION place_bid(p_property_id UUID, p_amount NUMERIC)
RETURNS VOID AS $$
DECLARE
    current_price NUMERIC;
BEGIN
    -- Get current price (lock property row to prevent race conditions)
    SELECT price INTO current_price FROM properties WHERE id = p_property_id FOR UPDATE;
    ...
    VALUES (p_property_id, auth.uid(), p_amount);

    -- Update Property Price
    UPDATE properties SET price = p_amount WHERE id = p_property_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 9. Seed Initial Data (Mock Data for Mira Road & Mumbai Suburbs)
