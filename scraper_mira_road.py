...
def main():
    ...
    scraped_data = scrape_99acres(session)
    
    current_data = None
    historical_data = None
    
    if scraped_data and len(scraped_data) >= 5:
        logger.info(f"Using scraped data ({len(scraped_data)} records)")
        current_data = scraped_data
    else:
        logger.info("Scraping failed or insufficient data, using synthetic data")
        current_data, historical_data = generate_synthetic_data(num_records=50)
    ...
    if current_file:
        logger.info(f"📁 Output file: {current_file}")
